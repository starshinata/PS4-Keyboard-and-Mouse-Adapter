...
namespace PS4RemotePlayInterceptor
{
    ...
    public class Injector
    {
        ...
        public static int Inject(int emulationMode, string processName)
        {
            Log.Information("cheese");
            string dllToInject = "PS4RemotePlayDll.dll";

            Log.Information("PS4 remoteplay injection - Injector.cs");

            Log.Information("Injector.Inject {emulationMode:{0}||{1}, processName:{2},  dllToInject:{3}",
                emulationMode,
            ...
            // Find the process
            Process process = ProcessUtil.FindProcess(processName);
            if (process == null)
            {
