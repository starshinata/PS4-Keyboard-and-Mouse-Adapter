...
namespace junk
{
    ...
    public class Injector
    {
        ...
        public static int Inject(int emulationMode, string processName, string dllToInject)
        {
            ...
            _channelName = "PS4KMA" + DateTime.Now.ToString("yy-MM-dd hh:mm:ss");
            ...
            try
            {
                ...
                Log.Information("Injector.Inject() _channelName " + _channelName);

                // Inject dll into the process
                if (shouldInject)
                {
                    Log.Debug("Injector.Inject RemoteHooking.Inject start");
                    WRXInjector.Inject(injectionLibrary, process.Id);
            ...
            {
                Log.Error("ex.FileName:" + ex.FileName);
                ExceptionLogger.LogException("FileNotFoundException, this looks fatal", ex);
                throw ex;
            }
            catch (Exception ex)
            {
